...
export const authApi = {
  login: credentials =>
    apiCall('/api/auth/login', { method: 'POST', body: credentials }),
  register: data =>
    apiCall('/api/auth/register', { method: 'POST', body: data }),
  getProfile: () => apiCall('/api/auth/profile'),
  updateProfile: data =>
    apiCall('/api/auth/profile', { method: 'PUT', body: data }),
  forgotPassword: email =>
    apiCall('/api/auth/forgot-password', { method: 'POST', body: { email } }),
  resetPassword: data =>
    apiCall('/api/auth/reset-password', { method: 'POST', body: data }),
};
...
export const adminApi = {
  ...
  deleteAIModel: id =>
    apiCall(`/api/admin/ai-models/${id}`, { method: 'DELETE' }),
  toggleAIModel: id =>
    apiCall(`/api/admin/ai-models/${id}/toggle`, { method: 'PUT' }),
  getLogs: () => apiCall('/api/admin/logs'),
  getAllTickets: () => apiCall('/api/tickets/all'),
  updateBusinessPlan: (businessId, planType) =>
    apiCall(`/api/admin/business/${businessId}/plan`, {
      method: 'PUT',
      body: { planType },
    }),
...
export const chatApi = {
  ...
  demoChat: message =>
    apiCall('/api/chat/demo', { method: 'POST', body: { message } }),
};
