...
export default function PageLayout({ children }) {
  ...
  return (
    <>
      {/* Navigation */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'h-20 shadow-lg' : 'h-24'} bg-white/95 dark:bg-cosmic-950/95 backdrop-blur-sm border-b border-gray-200 dark:border-white/5`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-full flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-6">
            {menuItems.slice(0, 4).map((item) => (
              <Link key={item.href} href={item.href} className={`text-sm font-medium transition-colors hover:text-brand-600 dark:hover:text-brand-400 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                {item.label}
              </Link>
            ))}
            ...
            <Link href="/register" className="px-6 py-2.5 rounded-full bg-gradient-to-r from-brand-600 to-brand-500 text-white font-bold shadow-lg hover:scale-105 transition-all text-sm">
              ابدأ الآن
            </Link>
          </div>
