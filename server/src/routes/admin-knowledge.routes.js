...
// ------------------------------------------------------------------
// Secret trigger endpoints removed — migration completed and temporary tooling cleaned up.
...
router.post('/knowledge/pgvector-migrate', requirePermission('system:write'), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (flag) return res.json({ success: true, message: 'Migration already completed', finishedAt: flag.value });
  } catch (e) {
    // ignore DB read errors; proceed to safe no-op response below
  }

  // The safe migration tool has already been executed during deploy. No further action required here.
  return res.status(200).json({ success: true, message: 'No action required. Migration state checked.' });
}));

// Secret-trigger endpoints removed — migration finished and temporary tooling cleaned up.
